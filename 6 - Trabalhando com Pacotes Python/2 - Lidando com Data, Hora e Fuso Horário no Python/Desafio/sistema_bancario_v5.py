...
def log_operacao(funcao):
    ...
    def wrapper(*args, **kwargs):
        ...
        print("\n========== REGISTRO DE LOG ==========")
        print("Data operação:", data)
        print("Tipo operação:", nome_operacao)
...
def gerador_relatorio(transacoes, filtro=0):
    ...
    match(filtro):
        case 1:
            saques = [transacao for transacao in transacoes if transacao["tipo"] == Saque.__name__]
            for saque in saques:
                yield saque
        case 2:
            deposito = [transacao for transacao in transacoes if transacao["tipo"] == Deposito.__name__]
...
class Contalterador:
    ...
    def __next__(self):
        try:
            ...
            self.contador += 1
            return "Não há contas para processar" if not self.contas else f"""\
...
def filtrar_cliente(cpf, clientes):
    ...
    # Se encontrar, retorna o elemento na 1° posição (pois nao existem dois clientes com o mesmo CPF) se não, retorna None
...
def gerar_relatorio(clientes):
    ...
    filtro = int(input("Qual o filtro do relatório?\n0 - Todas operações\n1 - Somente saques\n2 - Somente depósitos\n =>"))
